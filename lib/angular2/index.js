...
module.exports = function generate(ctx) {
        'use strict';
        // console.log(ctx.app.handler('rest').adapter.getClasses());
        // Describe models and remove those blacklisted
        ctx.models = utils.describeModels(ctx.app);
        const modelsWithEntityId = {};
        Object.keys(ctx.models).forEach((model) => {
            if (ctx.models[model].sharedClass.ctor.settings.hasOwnProperty('entityId')) {
                modelsWithEntityId[model] = { entityId: ctx.models[model].sharedClass.ctor.settings.entityId }
            }
        })
        console.log('Models with entity: ', modelsWithEntityId);
        //console.log(ctx.models['Momentumuser'].sharedClass.ctor.settings.effectId);
        if (ctx.excludeModels.length > 0) {
            var exlude = ctx.excludeModels.split(",");
            exlude.forEach(modelName => {
                if (ctx.models[modelName]) {
                    delete ctx.models[modelName];
                    console.info('Excluding %s from being added to SDK ', modelName);
                }
        ...
        if (ctx.isNgrx === 'enabled' || ctx.isNgrx === 'orm') {
            schema = schema.concat([
                ...
                },
                {
                    ...
                    params: { modelsWithEntityId: JSON.stringify(modelsWithEntityId) }
                },
                {
            ...
            if (ctx.models[modelName].sharedClass.ctor.settings.sdk &&
                     ...
                     */
                    {
                        ...
                        params: {
                            ...
                            path: ctx.models[modelName].sharedClass.ctor.settings.http &&
                                ...
                                ctx.models[modelName].sharedClass.ctor.settings.http.path : ctx.models[modelName].sharedClass.ctor.settings.plural,
                ...
                if (ctx.isNgrx === 'enabled' || ctx.isNgrx === 'orm') {
                    ...
                    schema.push({
                        ...
                        params: {
                            isIo: ctx.isIo,
                            entityId: ctx.models[modelName].sharedClass.ctor.settings.entityId || null,
                            model: ctx.models[modelName],
                            modelName: modelName,
                            moduleName: ctx.moduleName,
                            loadAccessToken: ctx.loadAccessToken,
            ...
            config => {
                ...
                fs.writeFileSync(
                    ...
                    ejs.render(fs.readFileSync(
                            require.resolve(config.template), { encoding: 'utf-8' }),
                        config.params
                    )
                )
            }
        ...
        function getUserModelName() {
            ...
            Object.keys(ctx.models).forEach(modelName => {
                if (ctx.models[modelName].sharedClass.ctor.settings.base === 'User' ||
                    ...
                    userModelName.push(modelName);
                }
            });
            if (userModelName.length === 0) {
                return '';
            }
            if (ctx.authModel.length === 0) {
                return userModelName[0];
            }
            for (const m of userModelName) {
                if (m == ctx.authModel) {
                    return m;
                }
            }
